...
def get_pp_loc_part2(cell, kpts=None):
    ...
    if kpts is None or numpy.shape(kpts) == (3,):
        vpploc = vpploc[0]
    _ = logger.timer_debug1(cell, 'get_pp_loc_part2', *cput0)
    return vpploc
...
def get_pp_loc_part2_gamma(cell):
    ...
    for cn, fake_cell in fake_cells.items():
        neighbor_list = build_neighbor_list_for_shlpairs(fake_cell, cell)
...
def vpploc_part2_nuc_grad(cell, dm, kpts=None):
    '''
    Nuclear gradients of the short-range local part of
    ...
    for cn in range(1, 5):
        ...
        if fakecell.nbas > 0:
            neighbor_list = build_neighbor_list_for_shlpairs(fakecell, cell)
    ...
    grad *= -2.
    _ = logger.timer_debug1(cell, 'vpploc_part2_nuc_grad', *cput0)
    return grad


def _prepare_hl_data(fakecell, hl_blocks):
    offset = [0] * 3
    hl_table = numpy.zeros((len(hl_blocks),HL_TABLE_SLOTS), order='C', dtype=numpy.int32)
    hl_data = []
    ptr = 0
...
def _contract_ppnl(cell, fakecell, hl_blocks, ppnl_half, comp=1, kpts=None):
    ...
    for k, kpt in enumerate(kpts_lst):
        ...
        libpbc.contract_ppnl(
            fill, buf.ctypes.data_as(ctypes.c_void_p),
    ...
    if kpts is None or numpy.shape(kpts) == (3,):
        ppnl = ppnl[0]
    _ = logger.timer_debug1(cell, '_contract_ppnl', *cput0)
    return ppnl
...
def _contract_ppnl_nuc_grad(cell, fakecell, dms, hl_blocks, ppnl_half, ppnl_half_ip2,
    ...
    _ = logger.timer_debug1(cell, '_contract_ppnl_nuc_grad', *cput0)
    return grad_tot
...
def get_pp_nl(cell, kpts=None):
    ...
    fakecell.precision = numpy.sqrt(cell.precision)
...
def _int_vnl(cell, fakecell, hl_blocks, kpts, intors=None, comp=1):
    ...
    rcut = cell.rcut_by_shells().max() + fakecell.rcut_by_shells().max()
    ...
    _ = logger.timer_debug1(cell, '_int_vnl', *cput0)
    return out
