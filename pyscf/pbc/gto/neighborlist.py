...
class NeighborList:
    def __init__(self, cell, cell1=None, Ls=None,
                 ish_rcut=None, jsh_rcut=None, hermi=0,
                 precision=None):
        '''
        Build the neighbor list for shell pairs in periodic calculations.
        ...
        ish_env = np.asarray(cell._env, order='C', dtype=np.float64)
        nish = len(ish_bas)
        if ish_rcut is None:
        ...
        if cell1 is cell:
            ...
            jsh_env = np.asarray(cell1._env, order='C', dtype=np.float64)
            if jsh_rcut is None:
        ...
        self.Ls = np.asarray(Ls, order='C', dtype=np.float64)
        nimgs = len(Ls)
        ...
        self._this = nl
        self._finalizer = weakref.finalize(self, free_neighbor_list, self._this)


def build_neighbor_list_for_shlpairs(cell, cell1=None, Ls=None,
                                     ish_rcut=None, jsh_rcut=None, hermi=0,
                                     precision=None):
    return NeighborList(cell, cell1=cell1, Ls=Ls,
                        ish_rcut=ish_rcut, jsh_rcut=jsh_rcut, hermi=hermi,
                        precision=precision)


def free_neighbor_list(nl_c_pointer):
    libpbc.del_neighbor_list(ctypes.byref(nl_c_pointer))
...
class NeighborListOpt:
    def __init__(self, cell):
        ...
        self._finalizer = weakref.finalize(self, self._free, self._this)
    ...
    def build(self, cell=None, cell1=None, Ls=None,
        ...
        if (set_nl or set_optimizer) and self.nl is None:
            ...
            libpbc.NLOpt_set_nl(self._this, self.nl._this)
    ...
    def _free(self, nlopt_c_pointer):
        self.reset()
        libpbc.NLOpt_del(ctypes.byref(nlopt_c_pointer))
